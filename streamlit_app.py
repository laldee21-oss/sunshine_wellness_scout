...
if "selected_agent" not in st.session_state:
    st.session_state.selected_agent = "home_scout"
...
<style>
    ...
    .agent-card { text-align: center; padding: 1rem; border-radius: 12px; background: rgba(255,255,255,0.8); }
    .stButton>button { background-color: #ea580c; color: white; border-radius: 12px; font-weight: bold; width: 100%; }
</style>
...
with cols[0]:
    ...
    st.markdown("Find your perfect Florida longevity home")
    if st.button("Select Home Scout", key="home", use_container_width=True):
        st.session_state.selected_agent = "home_scout"
...
with cols[1]:
    ...
    st.markdown("Custom workouts for your goals")
    if st.button("Select Personal Trainer", key="trainer", use_container_width=True):
        st.session_state.selected_agent = "trainer"
    st.markdown("</div>", unsafe_allow_html=True)

st.markdown("---")

# === Agent Content ===
if st.session_state.selected_agent == "home_scout":
    st.markdown("### Wellness Home Scout")
...
LBL Wellness Solutions Team
...
elif st.session_state.selected_agent == "trainer":
    st.markdown("### 💪 Your AI Personal Trainer")
    st.write("Get a custom workout plan tailored to your goals, equipment, and experience — powered by Grok.")
    ...
    if st.button("Generate My Custom Workout Plan", type="primary"):
        with st.spinner("Your AI Personal Trainer is creating your plan..."):
            trainer_prompt = f"""
            You are a certified personal trainer with military discipline and a focus on longevity and wellness.
            ...
            Create a complete weekly workout plan in clear markdown format with:
            - Warm-up for each day
            - Main workout (exercises, sets, reps, rest)
            - Cool-down/stretching
            - Progression tips for next week
            - Motivation and form notes

            Make it encouraging, safe, and sustainable for long-term health.
            """
            ...
            try:
                response = client.chat.completions.create(
                    model="grok-4-1-fast-reasoning",
                    messages=[
                        {"role": "system", "content": "You are a certified personal trainer specializing in longevity and sustainable fitness."},
                ...
                st.success("Here's your custom weekly workout plan!")
                st.markdown(workout_plan)
            except Exception as e:
                st.error("Trainer is warming up... try again in a moment.")
